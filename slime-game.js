...
// ══════════════════════════════════════════════
// 🗡️ 巡邏系統(勇者鬥惡龍風)：四園區/角色等級永久保留/死亡花錢復活/武器店
// ══════════════════════════════════════════════

// 每層基礎數值(3小怪+1王)，各園區用mult係數放大，已用模擬驗證平衡
const SG_RAID_FLOOR_BASE_HP  = [22, 32, 44, 62];
const SG_RAID_FLOOR_BASE_ATK = [3, 4, 5, 7];

const SG_RAID_ZONES = [
  { id:'kaoguan',  name:'會館',   tier:1, mult:1.0, recLevel:1,
    enemies:[
      { emoji:'💧', name:'漏水怪',   desc:'從水管縫隙滲出的黏稠水漬' },
      { emoji:'⚡', name:'跳電怪',   desc:'亂竄的電流噴出火花' },
      { emoji:'🔩', name:'卡榫怪',   desc:'卡死門鎖與鉸鏈的頑固鐵鏽' },
      { emoji:'🦀', name:'鏽蝕王',   desc:'佔據機房許久的鏽蝕巨獸(BOSS)', isBoss:true },
    ] },
  { id:'yashe',    name:'雅舍',   tier:2, mult:2.5, recLevel:9,
    enemies:[
      ...
      { emoji:'🌊', name:'水管魔',   desc:'盤據雅舍地下管線的水患之王(BOSS)', isBoss:true },
    ] },
  { id:'yanjiu',   name:'研究園', tier:3, mult:4.2, recLevel:18,
    enemies:[
      { emoji:'⚡', name:'靜電怪',   desc:'精密儀器旁亂竄的靜電火花' },
      { emoji:'💨', name:'塵爆怪',   desc:'粉塵濃度超標引發的危險氣旋' },
      { emoji:'🔬', name:'儀器怪',   desc:'校正失準、瘋狂亂動的實驗設備' },
      { emoji:'🌀', name:'斷電巨獸', desc:'吞噬整棟研究園電力的災厄(BOSS)', isBoss:true },
    ] },
  { id:'zhizun',   name:'智尊',   tier:4, mult:6.0, recLevel:28,
    enemies:[
      { emoji:'🛗', name:'電梯怪',   desc:'卡在樓層間、脾氣暴躁的升降機關' },
      { emoji:'📹', name:'監控怪',   desc:'畫面雪花、四處亂竄的監視幽靈' },
      { emoji:'🚪', name:'門禁怪',   desc:'拒絕所有人卡的固執門禁系統' },
      { emoji:'💥', name:'系統崩潰體', desc:'智尊大樓智慧系統全面失控的核心(BOSS)', isBoss:true },
    ] },
];
...
// 武器店：薪水換「工程幣」(專屬貨幣，比例故意壓貴)，再用工程幣買武器永久疊加攻擊力
...
function sgDefaultState() {
  return {
    ...
    raid: {                 // 🗡️巡邏系統：薪水的真正出口，花掉就是花掉，不會變回產能
      ...
      charExp: 0,
      engineeringCoin: 0,    // 武器店專屬貨幣(薪水兌換，比例故意壓貴)
...
// ══════════════════════════════════════════════
// 🗡️ 巡邏系統 核心邏輯（勇者鬥惡龍風：四園區/角色等級/死亡花錢復活/武器店）
// ══════════════════════════════════════════════
...
function sgRaidSpawnEnemy() {
  const zone = sgRaidGetZone();
  const idx = sg.raid.floor - 1;
  const hp = Math.round(SG_RAID_FLOOR_BASE_HP[idx] * zone.mult);
  sg.raid.enemyHp = hp;
  sg.raid.enemyMaxHp = hp;
}

function sgRaidCurrentEnemyDef() {
  const zone = sgRaidGetZone();
  const idx = sg.raid.floor - 1;
  const base = zone.enemies[idx];
...
function sgRaidAttack() {
  ...
  const enemyDef = sgRaidCurrentEnemyDef();
  sg.raid.turnCount++;
...
function sgRaidAbandon() {
  if (!confirm('確定要放棄這次巡邏嗎？出勤費用不會退還，但已累積的等級/經驗值會保留。')) return;
  sg.raid.inRun = false;
  sg.raid.knockedOut = false;
  sgRaidLog('🚪 提早撤退');
  sgSave();
...
function sgRaidBuyWeapon(weaponId) {
  const w = SG_RAID_WEAPONS.find(x=>x.id===weaponId);
  if (!w || sg.raid.weaponsOwned[weaponId]) return;
  if ((sg.raid.engineeringCoin||0) < w.cost) { showToast('工程幣不夠'); return; }
  sg.raid.engineeringCoin -= w.cost;
  sg.raid.weaponsOwned[weaponId] = true;
  showToast(`🛠️ 取得「${w.name}」！攻擊力+${w.atk}`);
  sgSave();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}
...
function sgRaidWeaponShopHtml() {
  ...
  const rate = sgRaidCoinRate();
  const rows = SG_RAID_WEAPONS.map(w => {
    ...
    return `
      <div class="sg-colleague-card">
        ...
        <button class="sg-colleague-buy" ${owned||!canAfford?'disabled':''} onclick="sgRaidBuyWeapon('${w.id}')">${owned?'已擁有':'購買'}</button>
      </div>`;
  }).join('');
  return `
    <div class="sg-colleague-crystal-bar">🔩 工程幣：${sgFormatNum(coin)}　｜　武器總攻擊力加成：+${sgRaidWeaponAtkTotal()}</div>
    ...
    <div class="sg-colleague-grid">${rows}</div>
...
function sgRaidZoneSelectHtml() {
  ...
  const cards = SG_RAID_ZONES.map(z => {
    ...
    return `
      <div class="sg-colleague-card">
        <div class="sg-colleague-emoji">${z.enemies[3].emoji}</div>
  ...
  return `
    <div class="sg-colleague-crystal-bar">
      🧑‍🔧 等級 Lv.${lv}　｜　經驗值 ${sgFormatNum(sg.raid.charExp)}/${sgFormatNum(expNext)}　｜　武器加成 +${sgRaidWeaponAtkTotal()}攻擊
    </div>
    ...
    <div style="text-align:center;margin-top:10px;">
      <button class="sg-colleague-buy" style="max-width:200px;" onclick="sgRaidShowWeaponShop()">🛠️ 前往武器店</button>
    </div>`;
}
...
function sgRaidTabHtml() {
  ...
  if (sg._raidPendingSkillChoice && sg._raidPendingSkillChoice.length) {
    ...
    return `
      <div class="sg-colleague-crystal-bar">🚪 ${zone.name} 第 ${r.floor} 層　❤️ ${r.playerHp}/${r.playerMaxHp}</div>
  ...
  if (r.knockedOut) {
    ...
    const canAfford = sg.xp >= cost;
    return `
      <div class="sg-colleague-crystal-bar">🚪 ${zone.name} 第 ${r.floor} 層</div>
      <div class="sg-colleague-card" style="text-align:center;padding:24px;">
        <div style="font-size:40px;">💀</div>
        <div style="font-weight:600;margin:8px 0;">體力耗盡，倒地了</div>
        <div style="font-size:13px;color:#999;margin-bottom:12px;">花錢可以立即復活繼續，或先離開去放置賺夠錢再回來，等級與經驗值都不會不見。</div>
  ...
  return `
    <div class="sg-colleague-crystal-bar">🚪 ${zone.name} 第 ${r.floor}/4 層　｜　已學技能：${learnedHtml}</div>
