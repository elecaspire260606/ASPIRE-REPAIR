...
// ══════════════════════════════════════════════
// 🗡️ 巡邏系統(勇者鬥惡龍風)：四園區/角色等級永久保留/死亡花錢復活/武器坊(武器+防具+飾品)
// ══════════════════════════════════════════════

// 每場戰鬥基礎數值(4場遞增戰鬥+1王)，各園區用mult係數放大，已模擬驗證平衡
const SG_RAID_FLOOR_BASE_HP  = [18, 24, 32, 42, 62];
const SG_RAID_FLOOR_BASE_ATK = [2.5, 3.2, 4, 5, 7.5];
// 6層樓層配置：4戰鬥+1非戰鬥事件(固定第3層)+1王(固定最後一層)
const SG_RAID_FLOOR_LAYOUT = ['combat','combat','event','combat','combat','boss'];
// 樓層數(1-indexed) → 戰鬥數值陣列索引
...
const SG_RAID_ZONES = [
  ...
  { id:'zhizun',   name:'智尊',   tier:4, mult:7.2, recLevel:28, weakElement:'電',
    regularPool:[
      ...
      { emoji:'🚪', name:'門禁怪', desc:'拒絕所有人卡的固執門禁系統' },
    ],
    boss:{ emoji:'💥', name:'系統崩潰體', desc:'智尊大樓智慧系統全面失控的核心(BOSS)', isBoss:true } },
];

// 非戰鬥事件(固定在每園區第3層)
const SG_RAID_EVENTS = [
  { id:'loot', emoji:'🎁', title:'撿到耗材',
    resolve:()=>{ const gain=Math.round(200+Math.random()*300); sg.xp+=gain; return `撿到一批耗材零件，折算 ${gain} 薪水`; } },
  { id:'trap', emoji:'⚠️', title:'觸發陷阱',
    resolve:()=>{ const dmg=Math.min(sg.raid.playerHp-1, Math.round(sg.raid.playerMaxHp*0.15)); sg.raid.playerHp=Math.max(1, sg.raid.playerHp-dmg); return `踩到裸露電線，損失 ${dmg} 點HP`; } },
  { id:'rest', emoji:'🍵', title:'稍作休息',
...
function sgRaidCoinRate() { return 100000; } // 10萬薪水 = 1 工程幣

const SG_RAID_WEAPONS = [
  { id:'w1', emoji:'🔨', name:'鐵鎚',           atk:5,  cost:3  },
  { id:'w2', emoji:'⚡', name:'電擊虎鉗',       atk:12, cost:8  },
  { id:'w3', emoji:'🔧', name:'液壓扳手',       atk:22, cost:18 },
  { id:'w4', emoji:'🗡️', name:'鈦合金鑽頭',     atk:38, cost:35 },
  { id:'w5', emoji:'👑', name:'園區至尊工具組', atk:60, cost:60 },
];
const SG_RAID_ARMORS = [
  { id:'a1', emoji:'🦺', name:'工作背心', def:2,  cost:4  },
  { id:'a2', emoji:'⛑️', name:'防撞頭盔', def:5,  cost:12 },
  { id:'a3', emoji:'🛡️', name:'防護鋼板', def:10, cost:28 },
];
const SG_RAID_ACCESSORIES = [
  { id:'ac1', emoji:'🍀', name:'幸運護符', desc:'每次出勤第一次攻擊必定會心',                 cost:10 },
  { id:'ac2', emoji:'🩹', name:'急救包',   desc:'體力耗盡時，30%機率免費自動復活一次(每次出勤限一次)', cost:15 },
  { id:'ac3', emoji:'⏳', name:'加班獎章', desc:'每5回合額外造成一次1.5倍加成傷害',            cost:20 },
];
...
function sgRaidWeaponAtkTotal() {
  ...
  return SG_RAID_WEAPONS.filter(w => sg.raid.weaponsOwned[w.id]).reduce((s,w)=>s+w.atk, 0);
}
function sgRaidArmorDefTotal() {
  if (!sg.raid || !sg.raid.armorsOwned) return 0;
  return SG_RAID_ARMORS.filter(a => sg.raid.armorsOwned[a.id]).reduce((s,a)=>s+a.def, 0);
}
function sgRaidExpToNext(level) { return Math.round(25 * Math.pow(level, 1.5)); }
function sgRaidGainExp(amount) {
  sg.raid.charExp += amount;
  const levelUps = [];
  while (sg.raid.charExp >= sgRaidExpToNext(sg.raid.charLevel)) {
    sg.raid.charExp -= sgRaidExpToNext(sg.raid.charLevel);
    sg.raid.charLevel++;
    levelUps.push(sg.raid.charLevel);
  }
  return levelUps;
}

// 出勤入場費：依園區tier而不同
function sgRaidEntryCost(zone) {
  const runs = (sg.raid && sg.raid.zoneRuns && sg.raid.zoneRuns[zone.id]) || 0;
  return Math.floor(1500 * zone.tier * Math.pow(1.35, runs));
}
// 復活費：中等痛感，隨園區tier與目前樓層浮動
function sgRaidReviveCost() {
  const zone = SG_RAID_ZONES.find(z=>z.id===sg.raid.selectedZone);
  if (!zone) return 0;
  return Math.round(500 * zone.mult * sg.raid.floor);
}
...
function sgDefaultState() {
  return {
    ...
    luckyClickCount: 0,
    raid: {                 // 🗡️巡邏系統：薪水的真正出口，花掉就是花掉，不會變回產能
      charLevel: 1,          // 角色等級(永久保留，不因死亡或轉生重置)
      charExp: 0,
      engineeringCoin: 0,    // 武器坊專屬貨幣(薪水兌換，比例故意壓貴)
      weaponsOwned: {},      // { weaponId: true }
      armorsOwned: {},       // { armorId: true }
      accessoriesOwned: {},  // { accessoryId: true }
      equippedAccessory: null, // 目前裝備的飾品id(單槽，可自由更換)
      zoneRuns: {},          // { zoneId: 累計出勤次數 }(影響入場費)
      zoneCleared: {},       // { zoneId: true }(是否曾破關過，僅顯示用)
      selectedZone: null,    // 目前所在園區id
      inRun: false,
      knockedOut: false,     // 體力耗盡、等待復活
      floor: 1,
      ...
      lastPlayerDmg: null,   // 最近一次玩家承受的傷害(飄字用)
      playerHp: 0,
      playerMaxHp: 0,
      enemyHp: 0,
      enemyMaxHp: 0,
...
function sgLoad() {
  ...
  // 巡邏系統：舊版MVP存檔(單園區/無等級)遷移為新版(四園區/角色等級/武器店)
  if (!sg.raid) sg.raid = sgDefaultState().raid;
  if (typeof sg.raid.charLevel !== 'number') sg.raid.charLevel = 1;
  if (typeof sg.raid.charExp !== 'number') sg.raid.charExp = 0;
  if (typeof sg.raid.engineeringCoin !== 'number') sg.raid.engineeringCoin = 0;
  if (!sg.raid.weaponsOwned) sg.raid.weaponsOwned = {};
  if (!sg.raid.zoneRuns) sg.raid.zoneRuns = {};
  if (!sg.raid.zoneCleared) sg.raid.zoneCleared = {};
  if (typeof sg.raid.selectedZone === 'undefined' || !SG_RAID_ZONES.find(z=>z.id===sg.raid.selectedZone)) {
  ...
  if (typeof sg.raid.equippedAccessory === 'undefined') sg.raid.equippedAccessory = null;
  if (!sg.raid.floorType) sg.raid.floorType = 'combat';
  if (typeof sg.raid.currentEnemyPick === 'undefined') sg.raid.currentEnemyPick = null;
  if (typeof sg.raid.currentEvent === 'undefined') sg.raid.currentEvent = null;
  if (typeof sg.raid.bossEnraged !== 'boolean') sg.raid.bossEnraged = false;
...
// ══════════════════════════════════════════════
// 🗡️ 巡邏系統 核心邏輯（勇者鬥惡龍風：四園區/角色等級/隨機小怪/事件樓層/王怪蓄力/裝備坊）
// ══════════════════════════════════════════════
...
function sgRaidSelectZone(zoneId) {
  sg.raid.selectedZone = zoneId;
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidBackToZoneSelect() {
  sg.raid.selectedZone = null;
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

// 出發：扣薪水、初始化本次出勤狀態(角色等級/裝備維持永久)
function sgRaidStart() {
  ...
  sg.xp -= cost;
  sg.raid.zoneRuns[zone.id] = (sg.raid.zoneRuns[zone.id]||0) + 1;
  sg.raid.inRun = true;
  sg.raid.knockedOut = false;
  sg.raid.floor = 1;
  sg.raid.playerMaxHp = sgRaidCharMaxHp();
  sg.raid.playerHp = sg.raid.playerMaxHp;
  sg.raid.skills = [];
  sg.raid.turnCount = 0;
  sg.raid.burnStacks = 0;
  sg.raid.usedFirstAttackCrit = false;
  sg.raid.usedAccessoryRevive = false;
  sg.raid.lastEnemyDmg = null;
  sg.raid.lastPlayerDmg = null;
  sg.raid.log = [];
  sgRaidLog(`🚪 前往${zone.name}巡邏，支付了 ${sgFormatNum(cost)} 薪水`);
  sgRaidEnterFloor();
  sgSave();
  sgUpdateNumbers();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
...
function sgRaidEnterFloor() {
  ...
  sg.raid.floorType = type;
  sg.raid.lastEnemyDmg = null;
  sg.raid.lastPlayerDmg = null;

  if (type === 'event') {
    sg.raid.currentEnemyPick = null;
    const ev = SG_RAID_EVENTS[Math.floor(Math.random()*SG_RAID_EVENTS.length)];
    const resultText = ev.resolve();
    sg.raid.currentEvent = { emoji:ev.emoji, title:ev.title, resultText };
    sgRaidLog(`${ev.emoji} ${ev.title}：${resultText}`);
    return;
  }
...
function sgRaidKnockOut() {
  // 🩹急救包：30%機率免費自動復活一次(每次出勤限一次)
  if (sg.raid.equippedAccessory === 'ac2' && !sg.raid.usedAccessoryRevive) {
    sg.raid.usedAccessoryRevive = true;
    if (Math.random() < 0.3) {
      sg.raid.playerHp = sg.raid.playerMaxHp;
      sgRaidLog(`🩹 急救包發動！免費自動復活`);
      sgSave();
      document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
      return;
    }
  }
  sg.raid.knockedOut = true;
  sgRaidLog(`💀 體力耗盡，倒地了……`);
  sgSave();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidRevive() {
  const cost = sgRaidReviveCost();
  if (sg.xp < cost) { showToast('薪水不夠支付復活費用，先去放置賺一下吧'); return; }
  sg.xp -= cost;
  sg.raid.knockedOut = false;
  sg.raid.playerHp = sg.raid.playerMaxHp;
  sgRaidLog(`💰 支付 ${sgFormatNum(cost)} 薪水，重新振作！`);
  sgSave();
  sgUpdateNumbers();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}
...
function sgRaidWinFloor(enemyDef) {
  const zone = sgRaidGetZone();
  const expGain = sg.raid.enemyMaxHp; // 經驗值與敵人強度掛鉤
  const levelUps = sgRaidGainExp(expGain);
  sgRaidLog(`✅ 擊敗 ${enemyDef.name}！獲得 ${expGain} 經驗值`);
  levelUps.forEach(lv => { sgRaidLog(`🌟 等級提升！Lv.${lv}`); });
  if (levelUps.length) showToast(`🌟 角色升級到 Lv.${sg.raid.charLevel}！`);

  if (enemyDef.isBoss) {
    sgRaidWinRun(zone, enemyDef);
    return;
  }
  sg.raid.floor++;
  sgRaidEnterFloor();
  if (sg.raid.floorType === 'combat' || sg.raid.floorType === 'boss') {
...
function sgRaidWinRun(zone, bossDef) {
  sg.raid.inRun = false;
  sg.raid.zoneCleared[zone.id] = true;
  const runs = sg.raid.zoneRuns[zone.id] || 1;
  const assetReward = Math.round((15 + runs * 3) * zone.tier);
  sg.assets = (sg.assets||0) + assetReward;
  sgRaidLog(`🏆 擊退${bossDef.name}！巡邏完成，獲得 ${assetReward} 資產`);
  sgSave();
  sgUpdateNumbers();
  showToast(`🏆 ${zone.name}巡邏完成！獲得 ${assetReward} 🏦資產`);
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidAbandon() {
  if (!confirm('確定要放棄這次巡邏嗎？出勤費用不會退還，但已累積的等級/經驗值/裝備都會保留。')) return;
  sg.raid.inRun = false;
  sg.raid.knockedOut = false;
  sgRaidLog('🚪 提早撤退');
  sgSave();
...
// 武器坊：薪水兌換工程幣
function sgRaidExchangeCoin() {
  const rate = sgRaidCoinRate();
  if (sg.xp < rate) { showToast('薪水不夠兌換1枚工程幣'); return; }
  const coins = Math.floor(sg.xp / rate);
  sg.xp -= coins * rate;
  sg.raid.engineeringCoin = (sg.raid.engineeringCoin||0) + coins;
  showToast(`🔁 兌換了 ${coins} 枚工程幣`);
  sgSave();
  sgUpdateNumbers();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidBuyItem(kind, id) {
  const list = kind==='weapon' ? SG_RAID_WEAPONS : kind==='armor' ? SG_RAID_ARMORS : SG_RAID_ACCESSORIES;
  const ownedKey = kind==='weapon' ? 'weaponsOwned' : kind==='armor' ? 'armorsOwned' : 'accessoriesOwned';
  const item = list.find(x=>x.id===id);
  if (!item || sg.raid[ownedKey][id]) return;
  if ((sg.raid.engineeringCoin||0) < item.cost) { showToast('工程幣不夠'); return; }
  sg.raid.engineeringCoin -= item.cost;
  sg.raid[ownedKey][id] = true;
  const bonusText = kind==='weapon' ? `攻擊力+${item.atk}` : kind==='armor' ? `防禦力+${item.def}` : item.desc;
  showToast(`🛠️ 取得「${item.name}」！${bonusText}`);
  sgSave();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidEquipAccessory(id) {
  sg.raid.equippedAccessory = (sg.raid.equippedAccessory === id) ? null : id;
  sgSave();
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidShowWeaponShop() {
  sg._raidShowWeaponShop = true;
  document.getElementById('sgTabContent').innerHTML = sgTabContentHtml();
}

function sgRaidWeaponShopHtml() {
  const coin = sg.raid.engineeringCoin || 0;
  const rate = sgRaidCoinRate();

  const weaponRows = SG_RAID_WEAPONS.map(w => {
    const owned = !!sg.raid.weaponsOwned[w.id];
    const canAfford = coin >= w.cost;
    return `<div class="sg-colleague-card">
      <div class="sg-colleague-emoji">${w.emoji}</div>
      <div class="sg-colleague-name">${w.name}</div>
      <div class="sg-colleague-bonus">攻擊力 +${w.atk}</div>
      <div class="sg-colleague-cost">🔩${w.cost} 工程幣</div>
      <button class="sg-colleague-buy" ${owned||!canAfford?'disabled':''} onclick="sgRaidBuyItem('weapon','${w.id}')">${owned?'已擁有':'購買'}</button>
    </div>`;
  }).join('');

  const armorRows = SG_RAID_ARMORS.map(a => {
    const owned = !!sg.raid.armorsOwned[a.id];
    const canAfford = coin >= a.cost;
    return `<div class="sg-colleague-card">
      <div class="sg-colleague-emoji">${a.emoji}</div>
      <div class="sg-colleague-name">${a.name}</div>
      <div class="sg-colleague-bonus">防禦力 +${a.def}(每次受擊固定減傷)</div>
      <div class="sg-colleague-cost">🔩${a.cost} 工程幣</div>
      <button class="sg-colleague-buy" ${owned||!canAfford?'disabled':''} onclick="sgRaidBuyItem('armor','${a.id}')">${owned?'已擁有':'購買'}</button>
    </div>`;
  }).join('');

  const accRows = SG_RAID_ACCESSORIES.map(ac => {
    const owned = !!sg.raid.accessoriesOwned[ac.id];
    const equipped = sg.raid.equippedAccessory === ac.id;
    const canAfford = coin >= ac.cost;
    return `<div class="sg-colleague-card">
      <div class="sg-colleague-emoji">${ac.emoji}</div>
      <div class="sg-colleague-name">${ac.name}${equipped?' ✅已裝備':''}</div>
      <div class="sg-colleague-bonus">${ac.desc}</div>
      <div class="sg-colleague-cost">🔩${ac.cost} 工程幣</div>
      ${owned
        ? `<button class="sg-colleague-buy" style="${equipped?'background:#999;':''}" onclick="sgRaidEquipAccessory('${ac.id}')">${equipped?'卸下':'裝備'}</button>`
        : `<button class="sg-colleague-buy" ${!canAfford?'disabled':''} onclick="sgRaidBuyItem('accessory','${ac.id}')">購買</button>`}
    </div>`;
  }).join('');
  ...
  return `
    ...
    <div style="font-weight:700;margin:14px 0 4px;">🛡️ 防具（永久疊加防禦力）</div>
    <div class="sg-colleague-grid">${armorRows}</div>
    <div style="font-weight:700;margin:14px 0 4px;">💍 飾品（單槽裝備，可自由更換）</div>
    <div class="sg-colleague-grid">${accRows}</div>
    <div style="text-align:center;margin-top:10px;">
      <button class="sg-colleague-buy" style="max-width:160px;background:#999;" onclick="sgRaidBackToZoneSelect()">🚪 返回園區選擇</button>
    </div>`;
}
...
function sgRaidZoneSelectHtml() {
  ...
  return `
    <div class="sg-colleague-crystal-bar">
    ...
    <div style="text-align:center;margin-top:10px;">
      <button class="sg-colleague-buy" style="max-width:200px;" onclick="sgRaidShowWeaponShop()">🛠️ 前往武器坊</button>
    </div>`;
}

function sgRaidDmgFloatHtml(kind) {
  const val = kind==='enemy' ? sg.raid.lastEnemyDmg : sg.raid.lastPlayerDmg;
  if (val === null || typeof val === 'undefined') return '';
  const cls = kind==='enemy' ? 'sg-raid-dmg-enemy' : 'sg-raid-dmg-player';
  const sign = kind==='enemy' ? '-' : '-';
  return `<div class="sg-raid-dmg-float ${cls}" key="${Date.now()}-${Math.random()}">${sign}${val}</div>`;
}

function sgRaidTabHtml() {
  const r = sg.raid;

  if (sg._raidShowWeaponShop) {
    return `<div style="text-align:right;"><button class="sg-colleague-buy" style="max-width:120px;background:#999;display:inline-block;" onclick="sg._raidShowWeaponShop=false;document.getElementById('sgTabContent').innerHTML=sgTabContentHtml();">✕ 關閉</button></div>` + sgRaidWeaponShopHtml();
  }

  if (!r.selectedZone) {
    return sgRaidZoneSelectHtml();
  }
